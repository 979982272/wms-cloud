...
## service name
APP_NAME=config-0.0.1-SNAPSHOT
JAR_FOLDER=eureka-config
...
case "$1" in
    ...
    start)
        nohup $JRE_HOME/bin/java -server -Xms128m -Xmx256m -XX:PermSize=64M -XX:MaxPermSize=128M -Dloader.path=./$JAR_FOLDER/lib -jar ./$JAR_FOLDER/$JAR_NAME --spring.cloud.inetutils.ignoredInterfaces=.*>/dev/null 2>&1 &
    ...
    restart)
        ...
        ;;
	log)
       tail -f /usr/local/cloud-server/logs/config.log
        ;;
    *)
        ## restart
